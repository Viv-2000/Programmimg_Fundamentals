...
################## all the inputs are case-sensitive, for eg: 'Titanic' is a movie name, and 'titanic' will be considered as new/different movie name ###################################
...
total_seats=[50,50,50]                                                                             #total_seats stores a list of total seats available for all movies


revenue_movie=[0,0,0]                                                                              #revenue_movie stores the revenue generated by all movies
...
def purchase_ticket():
    ...
    c_name=sys.stdin.readline().strip()               #c_name=customer name
    ...
    while True:       #this loop takes input for movie name, until a valid movie name is entered
        m_name=input("Please enter the movie name from the above movies list_").strip()                        #m_name=movie name
    ...
    m_index=movie_names.index(m_name)                #finding index of the input movie
    ...
    while input_invalid:          #this loop takes input for ticket types and respective quantities, untill all inputs are valid
            ...
            t_types=[c.strip() for c in t_types]                #t_types=final list of ticket type list entered
            
            print('Total tickets available for this movie are',total_seats[m_index])
            ...
            if t_num>total_seats[m_index]:                      #checking if total number of seats required are available or not
    ...
    total_seats[m_index]=total_seats[m_index]-t_num      #updating number of seats in the total seats list
    ...
    revenue_movie[m_index]+=total_cost          #adding the money to the corresponding total movie revenue list item
    ...
    for v,w in zip(t_types,num_t):        
        ...
        print('{:<15}{:>40}'.format('Ticket quantitiy',w))
...
def add_new():
    ...
    newmovie_list=[str1.strip() for str1 in newmovie_list]          #list comprehension that strips all the spaces from the input movies
    ...
    for i in range(0,len(newmovie_list)):                           #runs a loop to check if movie exists or not and add them if not existing
        ...
        elif newmovie_list[i] not in movie_names:
            movie_names.append(newmovie_list[i])              #adding movie name to the movie anmes list
            total_seats.append(50)                            #adding 50 seats for the respective movie
            revenue_movie.append(0)                           #adding the revenue list for the respective movie
...
def display_movies():
    ...
    for x in range(0,len(movie_names)):
        print(x+1,movie_names[x].center(48),str(total_seats[x]).center(30),'\n')                #str.center() is used on int by printing the string value of the number
...
def most_popular():                                                                             
    ...
    money=revenue_movie[0]                     #money is a variable to store the highest revenue in money
    for y in range(0,len(revenue_movie)-1):    #for loop to match value of money to the highest amount in revenue_movie list
        if money<=revenue_movie[y+1]:
            money=revenue_movie[y+1]           #storing highest amount of money in money
        else:
            continue
    most_pop=revenue_movie.index(money)        #storing index of the most selling film
    print("The most popular movie is",movie_names[most_pop],"with an astonishing revenue of $",revenue_movie[most_pop])
...
def movie_records():    
    ...
    for z,k in zip(movie_dict,revenue_movie):    #zip() used as number of movies and revenue list for movies will always have same number of elements
                                                 #zip() is used to iterate through multiple lists or dictionaries simultaneously
        print('{0:<15}{1:>15}{2:>15}{3:>15}{4:>15}{5:>15}{6:>15}'.format(z,movie_dict[z]['adult'],movie_dict[z]['child'],movie_dict[z]['senior'],movie_dict[z]['student'],movie_dict[z]['concession'],k))
